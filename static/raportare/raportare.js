let form = [
  ...
  {
    "label": "Dacă ați fost anunțat, vi s-au precizat",
    "type": "checkbox",
    "name": "precizat",
    "options": [
      "cauzele",
...
function getReports(reportsArray) {
  let url = '../api/raport'
  fetch(url, {
    method: 'put',
...
function getNamesFromSelect() {
  let namesArray = [];

  function getNames() {
    for (let i = 0; i < form.length; i++) {
      namesArray.push(form[i].name);
    }
  }
  getNames();
  namesArray.forEach((name) => {
    document.querySelector("#raports-names").innerHTML += `
        <option value="${name}">${name}</option>
        `
  })
}

document.querySelector("#raports-names").addEventListener("change", () => {
  let reportSelect = document.querySelector("#raports-names");
  document.querySelector("#raports-input").innerHTML += `<div class="query-item badge badge-primary d-flex justify-content-center align-items-center">${reportSelect.value}</div>`;
  reportSelect.value = '';
})

function generateReport() {
  let reportDivs = document.querySelectorAll(".query-item");
  let reportKeys = {};

  reportDivs.forEach((div) => {
    reportKeys[div.innerText] = true;
  });

  getReports(reportKeys);
}
