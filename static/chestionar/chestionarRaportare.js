...
function changejudet(obj) {
  obj.value;
  let options = '<option value="">Alege</option>';
...
function changelocalitate(obj) {
  let localitate = obj.value;
  let options = '<option value="">Alege</option>';
  let spitale = adreseSpitale[obj.parentElement.querySelector(`[name='${obj.name.replace("localitate", "judet")}']`).value][localitate];
  for (let i = 0; i < spitale.length; i++) {
